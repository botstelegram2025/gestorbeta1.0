...
class DatabaseManager:
    def __init__(self):
        ...
        if self.database_url:
            # Mostra só início da URL para debug
...
_Obrigado por escolher nossos serviços!_ ✨""",
    ...
    def cadastrar_cliente(self, nome, telefone, pacote, valor, servidor, vencimento, chat_id_usuario=None, info_adicional=None):
        ...
        if chat_id_usuario is None:
            chat_id_usuario = int(os.getenv('ADMIN_CHAT_ID', '1460561546'))
    ...
    def marcar_mensagem_processada(self, fila_id, sucesso, erro=None):
        """Marca mensagem como processada ou incrementa tentativa"""
        try:
            with self.get_connection() as conn:
                with conn.cursor() as cursor:
                    if sucesso:
    ...
    def buscar_mensagens_fila_cliente(self, cliente_id, apenas_pendentes=True):
        ...
        try:
            ...
            logger.error(f"Erro ao buscar mensagens da fila do cliente %s: %s", cliente_id, e)
            raise
